...
function frame_processing() {
        ...
        if (is_within_frame) {
            ...
            if ((dx_crop >= CROP_SIZE_REQUAREMENT) && (dy_crop >= CROP_SIZE_REQUAREMENT)) {
                ...
                if (clarity_relative < 1.0) {
                    status(STATUS_MAKE_PASSPORT_CLARITY);
                }
                
                if (brightness_relative < 1.0) {
                    status(STATUS_MAKE_PASSPORT_BRIGHTNESS);
                }
            ...
            if (is_frame_wider) {
                ...
                if (in_zone) {
                    ...
                    status(STATUS_EDGES_UP_DOWN_TO_ZONES);
                }
            } else {
                ...
                if (in_zone) {
                    ...
                    status(STATUS_LEFT_RIGHT_DOWN_TO_ZONES);
                }
            }
        } else {
            status(STATUS_PASSPORT_TO_FRAME);
        }
          
    } else {
        status(STATUS_PASSPORT_TO_CAMERA);
    }
    ...
    if (all_ok) {
        ...
        if (is_slow_motion) {
            ...
            status(STATUS_DONT_MOVE_PASSPORT);
        }
    }
